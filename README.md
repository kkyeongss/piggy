...
- **대시보드** — 분류별 지출(도넛 차트·합계 표시), 이번 달 수입 내역, 사용 가능 금액, 저축 누적, 주간 지출 추이(집계 기준 툴팁 ⓘ)
- **수입/지출/저축 내역** — 날짜별 그룹 리스트, 유형(수입·지출·저축) 탭 필터, 월 이동, 클릭 시 수정
- **달력** — 월 달력에 날짜별 거래 전체 표시, 날짜 클릭으로 입력·내역 클릭으로 수정, 일자 모달에서 ‹ › 화살표 또는 키보드 ← → 로 이전·다음 날 이동, 스페이스바로 빠른 추가
- **저축 유형** — 수입/지출과 별도로 저축을 독립 유형으로 관리, 지출 집계·사용 가능 금액 계산에서 제외
- **카테고리 / 지출방법 관리** — 수입·지출·저축 카테고리와 결제수단 CRUD, 카테고리명 변경 시 연결된 모든 거래 내역 자동 동기화
- **빠른 입력 모달** — 모든 화면에서 공통으로 쓰는 거래 입력/수정 컴포넌트
- **다크 모드** — 설정에서 토글, 선택값은 DB에 저장
- **회원가입 / 로그인** — 세션 기반 인증, 사용자별 데이터 완전 격리, 아이디 중복 검사(인증번호 발송 전), 아이디 영숫자 자동 변환
...
- 모든 데이터는 로컬 `./data/piggy.mv.db`(H2 파일 모드)에 저장되며 git에는 올리지 않습니다.
- 카테고리명을 변경하면 연결된 모든 거래 내역의 카테고리도 자동으로 업데이트됩니다.
- 카테고리·지출방법을 **삭제**해도 기존 거래 기록의 이름은 그대로 보존됩니다.
- 휴대폰 인증은 현재 개발용(`devCode` 응답)으로 동작하며 실제 SMS는 연동되어 있지 않습니다.
- 편의를 위해 로그인 화면에서 **바로 입장하기** 버튼으로 즉시 진입할 수 있습니다.
- H2는 `DB_CLOSE_ON_EXIT=FALSE;DB_CLOSE_DELAY=-1` 옵션으로 커넥션 유휴 시에도 파일을 열린 상태로 유지합니다.
